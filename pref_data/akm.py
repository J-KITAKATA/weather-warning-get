...
# エリアのキーとコード
area_id = {"AW":"013011", "AE":"013012", "AS":"013013", "KT":"013020", "MN":"013031", "MS":"013032"}
...
# キャッシュを保存するファイル名
CACHE_FILE = "./cache/aba_Kita_Mong-H_cache.json"
# フォルダを作成（すでにあればスルー）
# os.makedirs(os.path.dirname(CACHE_FILE), exist_ok=True)

safe_text = "警報・注意報の発表なし" # 発表されていない時用の出力

pn = "北海道 網走・北見・紋別地方" # 地域・エリアの名前
...
def pros(url, area, n_time):
    ...
    if area == None or area == "":
        # 網走地方
        p_data ="**" + pn + " 網走地方" + "**\n"
        target_code = "013010" # 網走地方のターゲットコード
        ...
        # 北見地方
        p_data = p_data + "\n" + "**" + pn + " 北見地方" +"**\n"
        target_code = "013020" # 北見地方のターゲットコード
        ...
        # 紋別地方
        p_data = p_data + "\n" +"**" + pn + " 紋別地方" +"**\n"
        target_code = "013030" # 紋別地方のターゲットコード
        ...
        # 網走西部
        p_data = "**" + pn + " 網走西部**\n"
        target_code = area_id[area] # 網走西部のターゲットコード
        ...
        # 重複を削除
        atn_data = list(set(atn_data))
        warn_data = list(set(warn_data))
        S_warn_data = list(set(S_warn_data))
        ...
        # 網走東部
        p_data = "**" + pn + " 網走東部**\n"
        target_code = area_id[area] # 網走東部のターゲットコード
        ...
        # 網走南部
        p_data = "**" + pn + " 網走南部**\n"
        target_code = area_id[area] # 網走南部のターゲットコード
        ...
        # 北見地方
        p_data = "**" + pn + " 北見地方**\n"
        target_code = area_id[area] # 北見地方のターゲットコード
        ...
        # 紋別北部
        p_data = "**" + pn + " 紋別北部**\n"
        target_code = area_id[area] # 紋別北部のターゲットコード
        ...
        # 紋別南部
        p_data = "**" + pn + " 紋別南部**\n"
        target_code = area_id[area] # 紋別南部のターゲットコード
