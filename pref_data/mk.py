...
# キャッシュを保存するファイル名
CACHE_FILE = "./cache/Miyakojima_cache.json"
# フォルダを作成（すでにあればスルー）
...
def pros(url, area, n_time):
    ...
    if area == None or area == "":
        # 宮古島地方
        p_data = "**" + pn + " **\n"
        target_code = "473000" # 宮古島地方のターゲットコード
        ...
        # 宮古島
        p_data = "**" + pn + " 宮古島**\n"
        target_code = area_id[area] # 宮古島のターゲットコード
        ...
        # 多良間島
        p_data = "**" + pn + " 多良間島**\n"
        target_code = area_id[area] # 多良間島のターゲットコード
