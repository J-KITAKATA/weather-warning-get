...
# エリアのキーとコード
area_id = {"HW":"015021", "HC":"015022", "HE":"015023"}
...
# キャッシュを保存するファイル名
CACHE_FILE = "./cache/Iburi_Hidaka-H_cache.json"
# フォルダを作成（すでにあればスルー）
# os.makedirs(os.path.dirname(CACHE_FILE), exist_ok=True)

safe_text = "警報・注意報の発表なし" # 発表されていない時用の出力

pn = "北海道 日高地方" # 地域・エリアの名前
...
def pros(url, area, n_time):
    ...
    if area == None or area == "":
        # 日高地方
        p_data = "**" + pn + "**\n"
        target_code = "015020" # 日高地方のターゲットコード
        ...
        # 日高西部
        p_data = "**" + pn + " 日高西部**\n"
        target_code = area_id[area] # 日高西部 のターゲットコード
        ...
        # 日高中部
        p_data = "**" + pn + " 日高中部**\n"
        target_code = area_id[area] # 日高中部 のターゲットコード
        ...
        # 日高東部
        p_data = "**" + pn + " 日高東部**\n"
        target_code = area_id[area] #日高東部 のターゲットコード
